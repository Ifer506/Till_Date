...
function Identifier$1(node, parent, tokenContext) {
  const parentType = parent.type;
...
function expandAliases(obj) {
  ...
  function add(type, func) {
    ...
    map.set(type, fn ? function (node, parent, stack, inForInit) {
      return fn(node, parent, stack, inForInit) ?? func(node, parent, stack, inForInit);
    } : func);
  }
...
function needsParens$1(node, parent, tokenContext, inForInit) {
  if (!parent) return false;
  if (isNewExpression$1(parent) && parent.callee === node) {
    if (isOrHasCallExpression(node)) return true;
  }
  if (isDecorator(parent)) {
    return !isDecoratorMemberExpression(node) && !(isCallExpression$1(node) && isDecoratorMemberExpression(node.callee)) && !isParenthesizedExpression(node);
  }
